...
cl_program b3OpenCLUtils_compileCLProgramFromString(cl_context clContext, cl_device_id device, const char* kernelSourceOrg, cl_int* pErrNum, const char* additionalMacrosArg , const char* clFileNameForCaching, bool disableBinaryCaching)
{
	...
	char* bla=0;
        ...
        wchar_t sCachedBinaryPath_w[B3_MAX_STRING_LENGTH];
        mbstowcs(sCachedBinaryPath_w, sCachedBinaryPath, strlen(sCachedBinaryPath) + 1);

        wchar_t binaryFileName_w[B3_MAX_STRING_LENGTH];
        mbstowcs(binaryFileName_w, binaryFileName, strlen(binaryFileName) + 1);

        CreateDirectory(sCachedBinaryPath_w,0);
		{

            HANDLE binaryFileHandle = CreateFile(binaryFileName_w,GENERIC_READ,0,0,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,0);
			...
			if (binaryFileValid)
			{
                ...
                wchar_t clFileNameForCaching_w[B3_MAX_STRING_LENGTH];
                mbstowcs(clFileNameForCaching_w, clFileNameForCaching, strlen(clFileNameForCaching) + 1);

                HANDLE srcFileHandle = CreateFile(clFileNameForCaching_w,GENERIC_READ,0,0,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,0);
				...
				if (srcFileHandle==INVALID_HANDLE_VALUE)
				{
					...
					for (int i=0;(srcFileHandle==INVALID_HANDLE_VALUE) && i<5;i++)
					{
                        ...
                        wchar_t relativeFileName_w[B3_MAX_STRING_LENGTH];
                        mbstowcs(relativeFileName_w, relativeFileName, strlen(relativeFileName) + 1);

                        srcFileHandle = CreateFile(relativeFileName_w,GENERIC_READ,0,0,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,0);
					}
